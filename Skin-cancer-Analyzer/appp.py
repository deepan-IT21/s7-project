...
MODELSPATH = r'F:\skin\Skin-cancer-Analyzer\Skin-cancer-Analyzer\models\model.h5'
DATAPATH = r'F:\skin\Skin-cancer-Analyzer\Skin-cancer-Analyzer\data'
...
def predict(x_test, model):
    ...
    ynew = np.round(Y_pred, 2)  # Round predictions to two decimal places
    ynew = ynew * 100  # Convert predictions to percentages
    y_new = ynew[0].tolist()  # Convert the predictions to a list format
    Y_pred_classes = np.argmax(Y_pred, axis=1)  # Get the class with the highest prediction
